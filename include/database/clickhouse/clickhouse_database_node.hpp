...
// opaque handle — clickhouse::Client* behind void*
using CHClientHandle = void*;

// per-database data for ClickHouse
// hierarchy: Server -> Databases -> Tables/Views (no schema layer)
class ClickHouseDatabaseNode : public IDatabaseNode, public ITableDataProvider {
public:
    ClickHouseDatabase* parentDb = nullptr;

    std::string name;

    std::unique_ptr<ConnectionPool<CHClientHandle>> connectionPool;
