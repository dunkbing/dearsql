...
namespace {
    void renderTablesSection(SQLiteDatabase *sqliteDb);
    void renderViewsSection(SQLiteDatabase *sqliteDb);
} // namespace

namespace SQLiteHierarchy {
    void renderSQLiteHierarchy(SQLiteDatabase *sqliteDb) {
        renderTablesSection(sqliteDb);
        renderViewsSection(sqliteDb);
    }

    void renderTableNode(SQLiteDatabase *sqliteDb, int tableIndex) {
        auto &app = Application::getInstance();
        const auto &table = sqliteDb->getTables()[tableIndex];
        ...
        if (ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(0)) {
            app.getTabManager()->createTableViewerTab(sqliteDb->getConnectionString(), table.name);
        }
        ...
        if (ImGui::BeginPopupContextItem(nullptr)) {
            if (ImGui::MenuItem("View Data")) {
                app.getTabManager()->createTableViewerTab(sqliteDb->getConnectionString(),
                                                          table.name);
            }
    ...
    void renderViewNode(SQLiteDatabase *sqliteDb, const int viewIndex) {
        auto &app = Application::getInstance();
        auto &view = sqliteDb->getViews()[viewIndex];
        ...
        if (ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(0)) {
            app.getTabManager()->createTableViewerTab(sqliteDb->getConnectionString(), view.name);
        }
    ...
    void renderTablesSection(SQLiteDatabase *sqliteDb) {
        ...
        const std::string tablesLabel =
            std::format("   Tables ({})###tables",
                        sqliteDb->getTables().size()); // 3 spaces for icon, ###tables for stable ID
        ...
        if (tablesOpen && !sqliteDb->areTablesLoaded()) {
            std::cout << "Tables node expanded and tables not loaded yet, attempting to load..."
                      << std::endl;
            sqliteDb->refreshTables();
        }

        if (tablesOpen) {
            if (sqliteDb->getTables().empty()) {
                if (!sqliteDb->areTablesLoaded()) {
                    ImGui::Text("  Loading...");
                } else {
                    ImGui::Text("  No tables found");
                }
            } else {
                for (int j = 0; j < sqliteDb->getTables().size(); j++) {
                    SQLiteHierarchy::renderTableNode(sqliteDb, j);
                }
            }
            ImGui::TreePop();
        }
    ...
    void renderViewsSection(SQLiteDatabase *sqliteDb) {
        ...
        const std::string viewsLabel =
            std::format("   Views ({})###views",
                        sqliteDb->getViews().size()); // 3 spaces for icon, ###views for stable ID
        ...
        if (viewsOpen && !sqliteDb->areViewsLoaded()) {
            std::cout << "Views node expanded and views not loaded yet, attempting to load..."
                      << std::endl;
            sqliteDb->refreshViews();
        }

        if (viewsOpen) {
            if (sqliteDb->getViews().empty()) {
                if (!sqliteDb->areViewsLoaded()) {
                    ImGui::Text("  Loading...");
                } else {
                    ImGui::Text("  No views found");
                }
            } else {
                for (int j = 0; j < sqliteDb->getViews().size(); j++) {
                    SQLiteHierarchy::renderViewNode(sqliteDb, j);
                }
            }
            ImGui::TreePop();
        }
